...
get_variable_values() {
    ...
    read -p "Name of the first bridge (e.g., br0) [default 'br0']: " BRIDGE1_NAME
    BRIDGE1_NAME=${BRIDGE1_NAME:-"br0"}

    read -p "Name of the second bridge (e.g., br1) [default 'br1']: " BRIDGE2_NAME
    BRIDGE2_NAME=${BRIDGE2_NAME:-"br1"}

    # br0
    echo "Interface 1 (connected to ${BRIDGE1_NAME}):"
    read -p "Enter IP address for interface 1 (e.g., 185.3.124.186): " IF1_IP
    read -p "Enter subnet mask for interface 1 (e.g., 255.255.255.240): " IF1_NETMASK
    read -p "Enter gateway for interface 1 (e.g., 185.3.124.177): " IF1_GATEWAY
    read -p "Enter DNS servers for interface 1 (comma-separated): " IF1_DNS

    # br1
    echo "Interface 2 (connected to ${BRIDGE2_NAME}):"
    read -p "Enter IP address for interface 2 (e.g., 192.168.50.17): " IF2_IP
    read -p "Enter subnet mask for interface 2 (e.g., 255.255.255.0): " IF2_NETMASK
    read -p "Enter gateway for interface 2 (e.g., 192.168.50.1): " IF2_GATEWAY
    read -p "Enter DNS servers for interface 2 (comma-separated): " IF2_DNS
}
